...
async function countPagesFromFiles(files: FileList): Promise<number> {
  ...
  for (const f of Array.from(files)) {
    ...
    if (name.endsWith(".pdf")) {
      ...
      total += 1; // ΓåÉ each image file (PNG, JPG) counts as 1 page
    }
  }
  return total;
}
...
// ΓöÇΓöÇ SVG Icon helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        >
          ...
          >
            ...
            <div>
              ...
              >
                Jonayskie Prints ΓÇö Effective January 1, 2026
              </div>
            </div>
          </div>
          <div
          ...
          >
              ...
              >
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      Online Payments ΓÇö GCash Only
                    </p>
                    <p
                      style={{
                        fontSize: ".82rem",
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      Cash Payments & Γé▒500+ Downpayment Requirement
                    </p>
                    <p
                      style={{
                        fontSize: ".82rem",
                    ...
                    >
                      ...
                      <strong>cash orders exceeding Γé▒500.00</strong> require a{" "}
                    ...
                    >
                      {[
                        "Downpayment must be settled via GCash prior to processing ΓÇö no exceptions.",
                ...
                <div>
                  ...
                  >
                    {[
                      ...
                      "Valid government-issued ID may be requested for orders above Γé▒1,000.00",
                    ].map((item, i) => (
                      <li
                        key={i}
                        style={{
            ...
            <TosSection
              ...
              body="Standard orders are processed within 1ΓÇô2 business days. Urgent requests may be accommodated based on availability and may incur additional rush fees. Jonayskie Prints is not liable for delays caused by incomplete file submissions, unclear instructions, or force majeure events."
            />
            <TosSection
              num="06"
            ...
            <TosSection
              ...
              body="All prices are in Philippine Peso (PHP). Final pricing is calculated based on paper size, color, quantity, and finishing options. Accepted payment methods: (1) Cash on pickup for any order amount. (2) GCash ΓÇö customers may pay the full amount online, or pay a 50% downpayment via GCash for cash orders exceeding Γé▒500.00 with the balance due on pickup. GCash is the only accepted online payment channel."
            />
            <TosSection
              num="08"
              title="Cancellations & Refunds"
              body="Orders may be cancelled without charge prior to processing. Once production has commenced, cancellations are not accepted and downpayments are forfeited. In cases where we cannot fulfill an order, a full refund will be issued within 3ΓÇô5 business days."
            />
            <TosSection
              num="09"
            ...
            >
              ...
              ΓÇö MonΓÇôSat, 8:00 AM ΓÇô 6:00 PM.
            </div>
          </div>
          <div
            style={{
          ...
          >
            {!hasScrolled && (
              ...
              >
                Γåô Scroll down to read all terms before accepting
              </p>
            )}
...
function DashboardPageInner() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      showToast(`Welcome back, ${user.first_name}! ≡ƒæï`, "success");
    }, 300);
  ...
  // ΓöÇΓöÇ NEW: unified multi-file handler for new order ΓöÇΓöÇ
  async function handleFileChange(files: FileList | null) {
    ...
    if (noService === "Print" || noService === "Photocopy") {
      ...
      if (totalPages > 0) {
        ...
        showToast(
          `${fileCount} file${fileCount > 1 ? "s" : ""} ΓÇö ${totalPages} total page${totalPages > 1 ? "s" : ""} ├ù ${copies} copies = ${totalPages * copies} total`,
          "success",
        );
      } else {
        setNoQuantity("");
  ...
  // ΓöÇΓöÇ NEW: unified multi-file handler for edit order ΓöÇΓöÇ
  ...
  return (
    <>
                ...
                <div className="p-grid">
                  {pricingCards.map((s) => (
                    <div key={s.name} className="p-card">
                      ...
                      <div className="p-price">Γé▒{s.price.toFixed(2)}</div>
                      ...
                      {recentOrders.map((o, idx) => (
                        ...
                        >
                          ...
                          <td className="ro-amount">
                            Γé▒{parseFloat(o.total_amount || "0").toFixed(2)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                  ...
                  <div className={`step-box ${step === 0 ? "active" : ""}`}>
                      ...
                      {showsCopies ? (
                        <div className="form-group">
                          ...
                          <div className="hint-text">
                            Upload PDF/DOCX in Step 3 ΓÇö pages will be
                            auto-counted
                          </div>
                          {noPdfPages > 0 && Number(noCopies) > 0 && (
                            <div className="copies-info">
                              Γ£ô {noPdfPages} pages ├ù {noCopies} copies ={" "}
                    ...
                    {noDelivery === "pickup" && (
                      <div className="form-group">
                        ...
                        />
                        <div className="hint-text">
                          Business hours: 8:00 AM ΓÇô 6:00 PM
                        </div>
                        {noPickupTime && (
                    ...
                    <div className="form-group">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          // if e.target.files is empty (user cancelled), do nothing ΓÇö keep previous noFiles
                        }}
                      />
                      {/* File list display */}
                      {noFiles && noFiles.length > 0 && (
                    ...
                    <div className="sum-box">
                            ...
                            if (sl === "print") {
                              unitPrice =
                                (noColorOption === "color"
                                  ? sp(prices.print_color, 2)
                                  : sp(prices.print_bw, 1)) * m;
                              priceLabel = `Γé▒${(noColorOption === "color" ? sp(prices.print_color, 2) : sp(prices.print_bw, 1)).toFixed(2)}/page${noPaperSize === "Long" ? " ├ù 1.2 (Long)" : ""}`;
                            } else if (sl === "photocopy") {
                              unitPrice = sp(prices.photocopying, 2) * m;
                              priceLabel = `Γé▒${sp(prices.photocopying, 2).toFixed(2)}/page${noPaperSize === "Long" ? " ├ù 1.2 (Long)" : ""}`;
                            } else if (sl === "scanning") {
                              unitPrice = sp(prices.scanning, 5) * m;
                              priceLabel = `Γé▒${sp(prices.scanning, 5).toFixed(2)}/page${noPaperSize === "Long" ? " ├ù 1.2 (Long)" : ""}`;
                            } else if (sl === "photo development") {
                              unitPrice = sp(prices.photo_development, 15);
                              priceLabel = `Γé▒${sp(prices.photo_development, 15).toFixed(2)}/photo`;
                            } else if (sl === "laminating") {
                              unitPrice = sp(prices.laminating, 20);
                              priceLabel = `Γé▒${sp(prices.laminating, 20).toFixed(2)}/item`;
                            }
                              ...
                              >
                                <span style={{ flex: 1 }}>
                                  ...
                                  {showsCopies && noPdfPages > 0 && (
                                    ...
                                    >
                                      {noPdfPages} pages ├ù {noCopies} copies ├ù Γé▒
                                      {(
                                  ...
                                  {!showsCopies && (
                                    ...
                                    >
                                      {effectiveQuantity} ├ù {priceLabel}
                                    </span>
                                  )}
                                </span>
                                <span
                      ...
                      {noPaymentMethod === "cash" && summaryTotal >= 500 && (
                        ...
                        >
                          ...
                          <span>
                            Your order exceeds <strong>Γé▒500.00</strong>. A{" "}
                            <strong>
                              50% GCash downpayment of Γé▒
                            ...
                            <strong>Γé▒{(summaryTotal * 0.5).toFixed(2)}</strong>{" "}
                            is due on pickup/delivery.
                          </span>
                        </div>
                      )}
                  ...
                  allOrders.map((o, idx) => {
                    const prev =
                      o.specifications.length > 75
                        ? o.specifications.slice(0, 75) + "ΓÇª"
                        : o.specifications;
                      ...
                      >
                        <div style={{ minWidth: 0, flex: 1 }}>
                          <div className="ord-id">
                            #{displayId(o.order_id)} ΓÇö {o.service}
                          </div>
                      ...
                      <div className="np-fadein">
                        ...
                        <div className="np-save-row">
                          ...
                          >
                            {profSubmitting ? (
                              <>
                                <div className="np-spinner" /> SavingΓÇª
                              </>
                            ) : (
                              <>
                                <svg
      ...
      {/* ΓòÉΓòÉ VIEW ORDER DETAILS MODAL ΓòÉΓòÉ */}
      {viewDetailsOrder && (
        <div
                  ...
                  if (sl === "print") {
                    unitPrice =
                      (colorOpt === "color"
                        ? sp(prices.print_color, 2)
                        : sp(prices.print_bw, 1)) * m;
                    priceNote = `${qty} page${qty > 1 ? "s" : ""} ├ù Γé▒${((colorOpt === "color" ? sp(prices.print_color, 2) : sp(prices.print_bw, 1)) * m).toFixed(2)}`;
                  } else if (sl === "photocopy") {
                    unitPrice = sp(prices.photocopying, 2) * m;
                    priceNote = `${qty} page${qty > 1 ? "s" : ""} ├ù Γé▒${(sp(prices.photocopying, 2) * m).toFixed(2)}`;
                  } else if (sl === "scanning") {
                    unitPrice = sp(prices.scanning, 5) * m;
                    priceNote = `${qty} page${qty > 1 ? "s" : ""} ├ù Γé▒${(sp(prices.scanning, 5) * m).toFixed(2)}`;
                  } else if (sl === "photo development") {
                    unitPrice = sp(prices.photo_development, 15);
                    priceNote = `${qty} photo${qty > 1 ? "s" : ""} ├ù Γé▒${sp(prices.photo_development, 15).toFixed(2)}`;
                  } else if (sl === "laminating") {
                    unitPrice = sp(prices.laminating, 20);
                    priceNote = `${qty} item${qty > 1 ? "s" : ""} ├ù Γé▒${sp(prices.laminating, 20).toFixed(2)}`;
                  }
                        ...
                        >
                          ...
                          >
                            ...
                            <span
                              style={{ fontSize: ".68rem", color: "#9ca3af" }}
                            >
                              ({qty} ├ù Γé▒{sp(prices.laminating, 20).toFixed(2)})
                            </span>
                          </span>
                          <span
                            style={{
      ...
      {/* ΓòÉΓòÉ EDIT ORDER MODAL ΓòÉΓòÉ */}
                ...
                {eoShowsCopies ? (
                  <div className="form-group">
                    ...
                    {eoPdfPages > 0 && Number(eoCopies) > 0 && (
                      <div className="copies-info">
                        Γ£ô {eoPdfPages} pages ├ù {eoCopies} copies ={" "}
              ...
              {eoShowsLam && (
                <div className="form-group">
                  <label className="check-label">
                    ...
                    />
                    Add Lamination (+Γé▒{sp(prices.laminating, 20).toFixed(2)}
                    /item)
                  </label>
                </div>
              )}
              ...
              <div className="form-group">
                ...
                {eoPdfPages > 0 && (
                  <div className="pdf-info">
                    <IC.PDF />
                    {eoShowsCopies
                      ? `${eoPdfPages} pages ├ù ${eoCopies} copies = ${eoPdfPages * Number(eoCopies)} total`
