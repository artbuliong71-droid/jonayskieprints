...
interface RegisterFormData {
  ...
  phone: string; password: string; confirmPassword: string; terms: boolean;
}
interface RegisterResponse {
  success: boolean; message: string; user_id?: number; redirect?: string;
}
...
const IconUser = () => (<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>);
...
const IconPhone = () => (<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 10.8 19.79 19.79 0 01.4 2.18 2 2 0 012.18 0H5.18a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 7.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 14.92z"/></svg>);
const IconLock = () => (<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>);
const IconEye = () => (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12C2.7 8.1 7 5 12 5s9.3 3.1 11 7c-1.7 3.9-6 7-11 7S2.7 15.9 1 12z"/><circle cx="12" cy="12" r="3"/></svg>);
const IconEyeOff = () => (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 3L21 21"/><path d="M10.6 10.6A2 2 0 0013.4 13.4"/><path d="M9.9 5.1A10.9 10.9 0 0112 5c5 0 9.3 3.1 11 7"/><path d="M6.2 6.2C4.2 7.4 2.7 9.1 1 12c1.7 3.9 6 7 11 7"/></svg>);
const IconCheck = () => (<svg width="10" height="8" viewBox="0 0 10 8" fill="none"><path d="M1 4L3.5 6.5L9 1" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>);
const IconXSmall = () => (<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{flexShrink:0}}><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>);
const IconNotifySuccess = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{flexShrink:0}}><polyline points="20 6 9 17 4 12"/></svg>);
const IconNotifyError = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{flexShrink:0}}><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>);
...
export default function RegisterPage() {
  ...
  const setField = <K extends keyof RegisterFormData>(key: K, value: RegisterFormData[K]) => {
  ...
  const validate = (): boolean => {
    ...
    else if (!/^[\d\s\-\+\(\)]{7,15}$/.test(formData.phone)) e.phone = "Invalid phone number format";
    if (!formData.password) e.password = "Password is required";
    else if (formData.password.length < PASSWORD_MIN_LENGTH) e.password = `Min. ${PASSWORD_MIN_LENGTH} characters`;
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/register", { method: "POST", body, headers: { Accept: "application/json" } });
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Phones landscape — reduce vertical spacing */
      ...
      <div className="page">
        <div className="card">
          <div className="card-header">
            <div className="brand-icon"><IconPrinter /></div>
            <div className="brand-name">Jonayskie Prints</div>
            <h1 className="card-title">Create Account</h1>
            <p className="card-subtitle">Join our community — it's completely free!</p>
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate>
              ...
              <div>
                ...
                <div className="input-wrapper">
                  ...
                  <input className={`form-input ${errors.firstName ? "has-error" : ""}`}
                    type="text" id="firstName" placeholder="Jane" autoComplete="given-name"
                    value={formData.firstName} onChange={(e) => setField("firstName", e.target.value)} />
                </div>
                {errors.firstName && <p className="field-error"><IconXSmall />{errors.firstName}</p>}
              </div>
              <div>
                ...
                <div className="input-wrapper">
                  ...
                  <input className={`form-input ${errors.lastName ? "has-error" : ""}`}
                    type="text" id="lastName" placeholder="Doe" autoComplete="family-name"
                    value={formData.lastName} onChange={(e) => setField("lastName", e.target.value)} />
                </div>
                {errors.lastName && <p className="field-error"><IconXSmall />{errors.lastName}</p>}
              </div>
            </div>
            ...
            <div className="form-group">
              ...
              {errors.email && <p className="field-error"><IconXSmall />{errors.email}</p>}
            </div>
            ...
            <div className="form-group">
              ...
              <div className="input-wrapper">
                ...
                <input className={`form-input ${errors.phone ? "has-error" : ""}`}
                  type="tel" id="phone" placeholder="+63 912 345 6789"
                  autoComplete="tel" inputMode="tel"
                  value={formData.phone} onChange={(e) => setField("phone", e.target.value)} />
              </div>
              {errors.phone && <p className="field-error"><IconXSmall />{errors.phone}</p>}
            </div>
            ...
            <div className="form-group">
              ...
              <div className="input-wrapper">
                ...
                <input className={`form-input with-toggle ${errors.password ? "has-error" : ""}`}
                ...
                <>
                  <div className="strength-bar-wrap">
                    {(["weak", "medium", "strong"] as const).map((lvl, i) => {
                      const levels: Record<PasswordStrength, number> = { none: 0, weak: 1, medium: 2, strong: 3 };
                      return <div key={lvl} className={`strength-segment ${i < levels[strength] ? `active ${strength}` : ""}`} />;
                    })}
                  </div>
              ...
              {errors.password && <p className="field-error"><IconXSmall />{errors.password}</p>}
            </div>
            ...
            <div className="form-group">
              ...
              <div className="input-wrapper">
                <span className="input-icon"><IconLock /></span>
                <input className={`form-input with-toggle ${errors.confirmPassword ? "has-error" : ""}`}
                  type={showConfirm ? "text" : "password"} id="confirmPassword"
                  placeholder="Re-enter your password" autoComplete="new-password"
                  value={formData.confirmPassword} onChange={(e) => setField("confirmPassword", e.target.value)} />
                <button type="button" className="toggle-btn"
                  onClick={() => setShowConfirm((v) => !v)} aria-label="Toggle confirm password">
                  {showConfirm ? <IconEyeOff /> : <IconEye />}
                </button>
              </div>
              {errors.confirmPassword && <p className="field-error"><IconXSmall />{errors.confirmPassword}</p>}
            </div>

            {/* Terms */}
            <div className="terms-row" onClick={() => setField("terms", !formData.terms)}>
              <div className={`custom-checkbox ${formData.terms ? "checked" : ""} ${errors.terms ? "err" : ""}`}>
            ...
            {errors.terms && (
              <p className="field-error" style={{ marginTop: "-0.5rem", marginBottom: "0.75rem" }}>
                <IconXSmall />{errors.terms}
              </p>
            )}
      ...
      <div className={`notification ${notification.type} ${notification.visible ? "visible" : ""}`}>
        {notification.type === "success" ? <IconNotifySuccess /> : <IconNotifyError />}
